...
class User extends CI_Controller {
	...
	public function registering(){
        ...
        //如果沒有先載入 UserModel 的話，$this->UserModel 是不會存在的喔
		$this->load->model("UserModel");

        if($this->UserModel->checkUserExist(trim($account))){ //檢查帳號是否重複
			$this->load->view('register',Array(
				"errorMessage" => "This account 有人用了唷!! is already in used." ,
				"account" => $account
			));
			return false;
		}
        ...
        //$this->load->view('register_success',Array("account" => $account));	//最後加上完成的 view
        //將 title 改為 "發文系統 - 會員註冊成功"
